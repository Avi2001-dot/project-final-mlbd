...
class TimeSeriesCrossValidatorError(Exception):
    """Custom exception for TimeSeriesCrossValidator errors."""
    pass
...
class TimeSeriesCrossValidator:
    ...
    def __init__(self, n_splits: int = 3, test_size: Optional[float] = None):
        ...
        if n_splits < 1:
            raise TimeSeriesCrossValidatorError("n_splits must be >= 1")
        ...
        if self.test_size <= 0 or self.test_size >= 1:
            raise TimeSeriesCrossValidatorError("test_size must be between 0 and 1")
        
        logger.info(
            f"TimeSeriesCrossValidator initialized with n_splits={n_splits}, "
    ...
    def split(
        ...
        if X is None or len(X) == 0:
            raise TimeSeriesCrossValidatorError("X cannot be None or empty")
        
        n_samples = len(X)
        fold_size = int(n_samples / (self.n_splits + 1))
        
        if fold_size < 1:
            raise TimeSeriesCrossValidatorError(
